...
!  Open file and set dimensions and attrributes 
   ...
   istat = nf_create("test_nc.nc",nf_clobber,ncid) 
   call check( nf_def_dim(ncid,"wavenumbers",nwv_h2o,id_wv) )
   call check( nf_def_var(ncid,"wvself",nf90_float,id_wv,idvar(1)) )

   call check( nf_put_att_text (ncid,varid(1),"units",4,"cm-1") )
   call check( nf_put_att_text (ncid,varid(1),"description",35,"Spectral domain of water vapor continuum"))

   call check( nf_enddef(ncid) )

!  Store variables 

   call check( nf_put_var(ncid, varid(1), wvn_h2o) )
