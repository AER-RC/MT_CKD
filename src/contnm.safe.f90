...
!
   ...
   DIMENSION SH2OT0(n_absrb),SP(n_absrb),FH2O(n_absrb),          &
   ...
   integer idvar(4)
   real wvn_h2o(nwv_h2o)
   integer istat
   character*78 text_arr(17)
   character*80 title
   character*780 copyright
   character*80 contact
   character*80 reference
...
!
   EQUIVALENCE (C0,SH2OT0,CN2T0,FCO2) , (C1,SP,CT1),             &
   &            (C2,FH2O,CT2)
!
   DATA P0 / 1013. /,T0 / 296. /
...
!   &     0.8500,0.8330,0.7810,0.7540,0.8180,                          &
   ...
   "Copyright �, Atmospheric and Environmental Research, Inc., 2022",    &
   " All rights reserved." ,&
   " This source code was developed as part of the LBLRTM software and ", &
   " is  designed for scientific and research purposes."  , &
   ...
   "  General reference: Mlawer et al. (2012), doi:10.1098/rsta.2011.0295  "/
      ...
      title = trim(text_arr(1))
      do ic=2,15
         if (ic.eq.2) then
            copyright=trim(text_arr(2))
         else
           copyright = trim(copyright)//trim(text_arr(ic))
        endif
      enddo
      ilen = len(copyright)

      contact = trim(text_arr(16))
      reference = trim(text_arr(17))
...
!     Get temperature exponent for SELF coefficient
      CALL SLP (V1C,V2C,DVC,NPTC,SP,v1ss,v2ss)
!
...
!  Output water vapor coefficients to netcdf file

!  Create frequency arr
   do iw=1,nwv_h2o
      wvn_h2o(iw) = v1ss+(iw+1)*dvc
   end do 

!  Open file and set dimensions and attrributes 
   !call check( nf_create("test_nc.nc",nf_clobber,ncid) )
   istat= nf_create("absco-ref_wv-mt-ckd.nc",nf_clobber,ncid) 
   istat= nf_def_dim(ncid,"nwvn",nwv_h2o,id_wv) 
   istat= nf_def_var(ncid,'wavenumbers',nf_double,1,id_wv,idvar(1)) 
   istat= nf_def_var(ncid,'self_absco_ref',nf_double,1,id_wv,idvar(2)) 
   istat= nf_def_var(ncid,'for_absco_ref',nf_double,1,id_wv,idvar(3))
   istat= nf_def_var(ncid,'self_texp',nf_double,1,id_wv,idvar(4))

   istat= nf_put_att_text (ncid,idvar(1),"units",4,"cm-1") 
   istat= nf_put_att_text (ncid,idvar(1),"description",40,"Spectral domain of water vapor continuum")
   istat= nf_put_att_text (ncid,idvar(2),"units",24,"1/(cm-1 molecules/cm**2)") 
   istat= nf_put_att_text (ncid,idvar(2),"description",50,"Water vapor self continuum absorption coefficients at 296K ")
   istat= nf_put_att_text (ncid,idvar(3),"units",24,"1/(cm-1 molecules/cm**2)") 
   istat= nf_put_att_text (ncid,idvar(3),"description",53,"Water vapor foreign continuum absorption coefficients")
   istat= nf_put_att_text (ncid,idvar(4),"units",15,"dimensionless") 
   istat= nf_put_att_text (ncid,idvar(4),"description",51,"Temperature exponent for water vapor self continuum")

   print *,'NF_GLOBAL ',NF_GLOBAL
   istat = nf_put_att_text (ncid,NF_GLOBAL,"title",80,title)
   istat = nf_put_att_text (ncid,NF_GLOBAL,"copyright",ilen,copyright)
   istat = nf_put_att_text (ncid,NF_GLOBAL,"contact",80,contact)
   istat = nf_put_att_text (ncid,NF_GLOBAL,"reference",80,reference)

   istat= nf_enddef(ncid) 

!  Store variables 

   istat= nf_put_var(ncid, idvar(1), wvn_h2o)
   istat= nf_put_var(ncid, idvar(2), sh2ot0(3:nwv_h2o+2)*1.0e-20)
   istat= nf_put_var(ncid, idvar(3), fh2o(3:nwv_h2o+2)*1.0e-20)
   istat= nf_put_var(ncid, idvar(4), sp(3:nwv_h2o+2))
...
!     --------------------------------------------------------------
!
SUBROUTINE SLP (V1C,V2C,DVC,NPTC,CP,v1ss,v2ss)
...
!
   COMMON /ABSORB/ V1ABS,V2ABS,DVABS,NPTABS,ABSRB(n_absrb)
   COMMON /SPOWER/ V1S,V2S,DVS,NPTS,SP(2003)
   DIMENSION CP(*)
!
   DVC = DVS
   v1ss = v1s
...
!
   COMMON /SPOWER/ V1,V2,DV,NPT,                                       &
   &         SP0000( 2),SP0001(50),SP0051(50),SP0101(50),SP0151(50),  &
   &         SP0201(50),SP0251(50),SP0301(50),SP0351(50),SP0401(50),  &
   &         SP0451(50),SP0501(50),SP0551(50),SP0601(50),SP0651(50),  &
   &         SP0701(50),SP0751(50),SP0801(50),SP0851(50),SP0901(50),  &
   &         SP0951(50),SP1001(50),SP1051(50),SP1101(50),SP1151(50),  &
   &         SP1201(50),SP1251(50),SP1301(50),SP1351(50),SP1401(50),  &
   &         SP1451(50),SP1501(50),SP1551(50),SP1601(50),SP1651(50),  &
   &         SP1701(50),SP1751(50),SP1801(50),SP1851(50),SP1901(50),  &
   &         SP1951(50),SP2001(1)
!
   DATA V1,V2,DV,NPT / -20.0, 20000.0, 10.0, 2003/
   DATA SP0000/                                                       &
