...
Instead of cloning, users can also download an MT_CKD [tarball](https://github.com/AER-RC/MT_CKD/releases/tag/4.1.1) and unpack it:

```
tar xvf cntnm_4.1.1.tar.gz
mv MT_CKD-4.1.1 cntnm
```
...
```
cd run_example/
ln -s ../cntnm_v4.1.1_linux_pgi_dbl cntnm # assuming v4.1.1 was built with PGI in double precision (linuxPGIdbl)
./cntnm (0, enter)
```
